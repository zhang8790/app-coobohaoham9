...
const STATUS_COLOR: Record<string, string> = { settled: 'var(--success-strong)', reversed: 'var(--danger)' }
const STATUS_BG: Record<string, string> = { settled: 'var(--success-soft)', reversed: 'var(--danger-soft)' }
...
export default function MerchantSettlements() {
  ...
  const S = {
    ...
    td: { padding: '12px 14px', fontSize: 13, borderBottom: '1px solid var(--border)' } as React.CSSProperties,
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
        {[
          { label: '累计已结算货款', val: yuan(summary.total_settled), color: 'var(--success-strong)' },
          { label: '结算笔数', val: summary.count, color: 'var(--primary)' },
          { label: '涉及门店数', val: summary.store_count, color: 'var(--info)' },
        ].map(c => (
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
          ...
          {FILTERS.map(f => (
            <button key={f.key} onClick={() => { setPage(0); setStatus(f.key) }}
              style={{ padding: '7px 16px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer',
                background: status === f.key ? 'var(--success-strong)' : 'var(--surface-2)', color: status === f.key ? '#fff' : 'var(--text-muted)',
                border: `1px solid ${status === f.key ? 'var(--success-strong)' : 'var(--border)'}` }}>
              {f.label}
            </button>
          ))}
        </div>
        <button onClick={handleBackfill} disabled={backfilling}
          style={{ padding: '8px 18px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: backfilling ? 'not-allowed' : 'pointer',
            background: 'var(--border)', color: 'var(--text)', border: '1px solid var(--border-soft)', opacity: backfilling ? 0.6 : 1 }}>
          {backfilling ? '补结算中…' : '历史补结算'}
        </button>
      </div>
      {backfillMsg && (
        <div style={{ padding: '10px 16px', borderRadius: 8, background: 'var(--surface-2)', border: '1px solid var(--border)', color: 'var(--text-muted)', fontSize: 13 }}>
          {backfillMsg}
        </div>
      )}
      ...
      <div style={S.card}>
        {loading ? (
          ...
          <div style={{ padding: 60, textAlign: 'center', color: 'var(--text-dim)' }}>暂无结算记录 ✓</div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 980 }}>
              <thead>
                <tr>
                  {['门店', '订单号', '订单全额', '豆付部分', '现金部分', '平台让利', '通道费', '应收货款', '状态', '结算时间'].map(h => (
              ...
              <tbody>
                {list.map(r => (
                  <tr key={r.id}>
                    <td style={{ ...S.td, color: 'var(--text)', fontWeight: 600 }}>
                      {r.stores?.name || '—'}
                      {r.stores?.wx_sub_mch_id && <div style={{ color: 'var(--text-dim)', fontSize: 11 }}>子商户号 {r.stores.wx_sub_mch_id}</div>}
                    </td>
                    <td style={{ ...S.td, color: 'var(--text-muted)' }}>{r.order_no || '—'}</td>
                    <td style={{ ...S.td, color: 'var(--text)' }}>{yuan(r.total_amount)}</td>
                    <td style={{ ...S.td, color: 'var(--accent-text)' }}>{yuan(r.tb_portion)}</td>
                    <td style={{ ...S.td, color: 'var(--text-muted)' }}>{yuan(r.cash_portion)}</td>
                    <td style={{ ...S.td, color: 'var(--warning)' }}>-{yuan(r.discount_pool)}</td>
                    <td style={{ ...S.td, color: 'var(--text-dim)' }}>-{yuan(r.channel_fee)}</td>
                    <td style={{ ...S.td, color: 'var(--success-strong)', fontWeight: 700 }}>{yuan(r.settle_amount)}</td>
                    <td style={S.td}>
                      <span style={{ color: STATUS_COLOR[r.status] || 'var(--text-muted)', fontSize: 12, fontWeight: 600, padding: '2px 8px', background: STATUS_BG[r.status] || 'var(--text-dim)22', borderRadius: 4 }}>
                        {STATUS_LABEL[r.status] || r.status}
                      </span>
                    </td>
                    <td style={{ ...S.td, color: 'var(--text-dim)' }}>{r.settled_at ? new Date(r.settled_at).toLocaleString('zh-CN') : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            {Array.from({ length: totalPages }, (_, i) => (
              <button key={i} onClick={() => setPage(i)}
                style={{ width: 32, height: 32, borderRadius: 6, border: 'none', cursor: 'pointer', fontSize: 13,
                  background: page === i ? 'var(--success-strong)' : 'var(--border)', color: page === i ? '#fff' : 'var(--text-muted)' }}>
                {i + 1}
              </button>
            ))}
          </div>
