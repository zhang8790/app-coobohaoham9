...
const STATUS_COLOR: Record<string, string> = { active: 'var(--success-strong)', draft: 'var(--text-dim)', paused: 'var(--warning)', expired: 'var(--text-muted)' }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <h2 style={{ color: 'var(--text)', fontSize: 24, fontWeight: 700 }}>优惠券管理</h2>
        <button
      ...
      {!loading && storeId && (
        <>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
            {[
              ...
              <button
                ...
                style={{
                  padding: '6px 14px',
                  background: filter === tab.key ? 'var(--success-strong)' : 'var(--surface-2)',
                  border: `1px solid ${filter === tab.key ? 'var(--success-strong)' : 'var(--border)'}`,
                  borderRadius: 6,
          ...
          {filtered.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 60, color: 'var(--text-dim)', fontSize: 14 }}>暂无优惠券，点击右上角新建</div>
          ) : (
              ...
              {filtered.map(coupon => (
                <div key={coupon.id} style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 12, padding: 20, position: 'relative', overflow: 'hidden' }}>
                  <div style={{
                    ...
                    padding: '4px 12px',
                    background: STATUS_COLOR[coupon.status] || 'var(--border-soft)',
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', marginBottom: 12 }}>
                    <div style={{ textAlign: 'center', flex: 1 }}>
                      <p style={{ color: 'var(--text)', fontSize: 18, fontWeight: 700 }}>{coupon.total}</p>
                      <p style={{ color: 'var(--text-dim)', fontSize: 12 }}>发放总量</p>
                    </div>
                    <div style={{ width: 1, background: 'var(--border)' }} />
                    <div style={{ textAlign: 'center', flex: 1 }}>
                      <p style={{ color: 'var(--success-strong)', fontSize: 18, fontWeight: 700 }}>{coupon.claimed_count}</p>
                      <p style={{ color: 'var(--text-dim)', fontSize: 12 }}>已领取</p>
                    </div>
                    <div style={{ width: 1, background: 'var(--border)' }} />
                    <div style={{ textAlign: 'center', flex: 1 }}>
                      <p style={{ color: 'var(--primary)', fontSize: 18, fontWeight: 700 }}>{coupon.total - coupon.claimed_count}</p>
                      <p style={{ color: 'var(--text-dim)', fontSize: 12 }}>剩余</p>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button
                      ...
                      style={{ flex: 1, padding: '8px', background: 'transparent', border: '1px solid var(--border-soft)', borderRadius: 6, color: 'var(--text-muted)', fontSize: 13, cursor: 'pointer' }}
                    >
                      {coupon.status === 'active' ? '下架' : '上架'}
                    </button>
                    <button
                      onClick={() => handleDelete(coupon.id)}
                      style={{ flex: 1, padding: '8px', background: 'transparent', border: '1px solid var(--danger)', borderRadius: 6, color: 'var(--danger)', fontSize: 13, cursor: 'pointer' }}
                    >
                      删除
                    </button>
                  </div>
      ...
      {showCreate && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }} onClick={() => setShowCreate(false)}>
          <div style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 16, padding: 24, width: 480, maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            <h3 style={{ color: 'var(--text)', fontSize: 18, fontWeight: 700, marginBottom: 20 }}>创建优惠券</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20 }}>
              <div>
                <label style={{ color: 'var(--text-muted)', fontSize: 13, display: 'block', marginBottom: 6 }}>优惠券名称 *</label>
                <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="如：新客立减5元" style={{ width: '100%', padding: '10px 12px', background: 'var(--bg)', border: '1px solid var(--border-soft)', borderRadius: 8, color: 'var(--text)', fontSize: 14, outline: 'none' }} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ color: 'var(--text-muted)', fontSize: 13, display: 'block', marginBottom: 6 }}>优惠类型</label>
                  <select value={form.discount_type} onChange={e => setForm({ ...form, discount_type: e.target.value })} style={{ width: '100%', padding: '10px 12px', background: 'var(--bg)', border: '1px solid var(--border-soft)', borderRadius: 8, color: 'var(--text)', fontSize: 14, outline: 'none' }}>
                    <option value="amount">满减券</option>
                    <option value="percent">折扣券</option>
                  </select>
                </div>
                <div>
                  <label style={{ color: 'var(--text-muted)', fontSize: 13, display: 'block', marginBottom: 6 }}>优惠金额/折扣 *</label>
                  <input type="number" value={form.discount_value} onChange={e => setForm({ ...form, discount_value: Number(e.target.value) })} placeholder="如：5（元）或 10（9折）" style={{ width: '100%', padding: '10px 12px', background: 'var(--bg)', border: '1px solid var(--border-soft)', borderRadius: 8, color: 'var(--text)', fontSize: 14, outline: 'none' }} />
                </div>
              </div>
              <div>
                <label style={{ color: 'var(--text-muted)', fontSize: 13, display: 'block', marginBottom: 6 }}>最低消费（元，0=无门槛）</label>
                <input type="number" value={form.min_amount} onChange={e => setForm({ ...form, min_amount: Number(e.target.value) })} placeholder="0" style={{ width: '100%', padding: '10px 12px', background: 'var(--bg)', border: '1px solid var(--border-soft)', borderRadius: 8, color: 'var(--text)', fontSize: 14, outline: 'none' }} />
              </div>
              <div>
                <label style={{ color: 'var(--text-muted)', fontSize: 13, display: 'block', marginBottom: 6 }}>发放数量 *</label>
                <input type="number" value={form.total} onChange={e => setForm({ ...form, total: Number(e.target.value) })} placeholder="如：100" style={{ width: '100%', padding: '10px 12px', background: 'var(--bg)', border: '1px solid var(--border-soft)', borderRadius: 8, color: 'var(--text)', fontSize: 14, outline: 'none' }} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <button onClick={handleCreate} disabled={submitting} style={{ flex: 1, padding: '10px', background: submitting ? 'var(--border-soft)' : 'var(--success-strong)', border: 'none', borderRadius: 8, color: 'white', fontSize: 14, fontWeight: 600, cursor: submitting ? 'not-allowed' : 'pointer' }}>{submitting ? '提交中…' : '创建'}</button>
            </div>
          </div>
        </div>
      )}
