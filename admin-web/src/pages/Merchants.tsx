...
export default function Merchants() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        <h1 style={{ color: 'var(--text)', fontSize: 22, fontWeight: 700, marginBottom: 4 }}>自营门店</h1>
        <p style={{ color: 'var(--text-dim)', fontSize: 14 }}>自营门店申请审核</p>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: 'var(--bg)' }}>
                {['自营门店名称', '联系人', '电话', '类型', '简介', '申请时间', '状态', '操作'].map(h => (
      ...
      {rejectModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 16, padding: 28, width: 440 }}>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: 14, marginBottom: 20 }}>自营门店：{rejectModal.name}</p>
            <textarea
              value={reason} onChange={e => setReason(e.target.value)}
              placeholder="请填写驳回理由，将通知申请自营门店..."
