...
export default function EmotionClaims() {
  ...
  const S = {
    ...
    th: { color: '#6B7280', fontSize: 12, fontWeight: 500, padding: '10px 16px', textAlign: 'left' as const, background: '#0B0F19', whiteSpace: 'nowrap' as const },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        <h1 style={{ color: '#E5E7EB', fontSize: 22, fontWeight: 700, marginBottom: 4 }}>确权治理</h1>
      ...
      {stats && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12 }}>
          <StatCard label="确权总数" value={String(stats.total)} accent="#C2410C" />
          <StatCard label="有效确权" value={String(stats.active)} accent="#10B981" />
          <StatCard label="已作废" value={String(stats.voided)} accent="#6B7280" />
          <StatCard label="有效贡献值 CV" value={stats.active_cv.toFixed(4)} accent="#6366F1" />
          <StatCard label="有效通宝 TB" value={stats.active_tb.toFixed(2)} accent="#F59E0B" />
          <StatCard label="未封禁用户" value={String(stats.active_users)} accent="#0EA5E9" />
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        ...
        <button onClick={() => setShowRules(v => !v)}
          style={{ padding: '8px 16px', background: 'transparent', border: '1px solid #1F2937', borderRadius: 6, color: '#9CA3AF', fontSize: 13, cursor: 'pointer' }}>
      ...
      {showRules && (
        <div style={{ ...S.card, padding: 0, overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {rules.length === 0 ? (
                <tr><td colSpan={6} style={{ ...S.td, textAlign: 'center', color: '#6B7280' }}>暂无规则版本</td></tr>
              ) : rules.map(r => (
                <tr key={r.version}>
                  <td style={{ ...S.td, color: '#E5E7EB', fontWeight: 600 }}>v{r.version}</td>
                  <td style={{ ...S.td, color: '#9CA3AF', fontSize: 13 }}>{fmt(r.announced_at)}</td>
                  <td style={{ ...S.td, color: '#9CA3AF', fontSize: 13 }}>{fmt(r.effective_at)}</td>
                  <td style={S.td}>
                    <span style={{ padding: '2px 10px', borderRadius: 20, fontSize: 12, fontWeight: 600, background: r.is_active ? '#10B98122' : '#374151', color: r.is_active ? '#10B981' : '#9CA3AF' }}>
                      {r.is_active ? '现行' : '历史'}
                    </span>
                  </td>
                  <td style={{ ...S.td, color: '#E5E7EB' }}>{r.note || '-'}</td>
      ...
      <div style={{ ...S.card, padding: 0, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={9} style={{ ...S.td, textAlign: 'center', color: '#6B7280' }}>加载中...</td></tr>
            ) : list.length === 0 ? (
              <tr><td colSpan={9} style={{ ...S.td, textAlign: 'center', color: '#6B7280' }}>暂无确权记录</td></tr>
            ) : list.map(c => (
              <tr key={c.id}>
                <td style={S.td}>
                  <div style={{ color: '#E5E7EB', fontWeight: 600 }}>{c.nickname || '侠客'}</div>
                  <div style={{ color: '#6B7280', fontSize: 12 }}>
                    {maskPhone(c.phone) || c.user_id.slice(0, 8)}
                    {c.user_is_banned && <span style={{ marginLeft: 6, color: '#EF4444', fontSize: 11 }}>已封禁</span>}
                  </div>
                </td>
                <td style={{ ...S.td, color: '#9CA3AF', fontSize: 12 }}>{c.order_no || '-'}</td>
                <td style={{ ...S.td, color: '#F59E0B', fontWeight: 600 }}>{c.tb_amount.toFixed(2)}</td>
                <td style={{ ...S.td, color: '#A5B4FC', fontWeight: 600 }}>{c.cv_amount.toFixed(4)}</td>
                <td style={{ ...S.td, color: '#E5E7EB' }}>{c.badge_text || c.badge_code || '-'}</td>
                <td style={{ ...S.td, color: '#9CA3AF', fontSize: 12 }}>v{c.rule_version || '?'}</td>
                <td style={S.td}>
                  {c.status === 'active' ? (
                    <span style={{ padding: '2px 10px', borderRadius: 20, fontSize: 12, fontWeight: 600, background: '#10B98122', color: '#10B981' }}>有效</span>
                  ) : (
                    <span style={{ padding: '2px 10px', borderRadius: 20, fontSize: 12, fontWeight: 600, background: '#374151', color: '#9CA3AF' }}>已作废</span>
                  )}
                </td>
                <td style={{ ...S.td, color: '#6B7280', fontSize: 12 }}>{new Date(c.created_at).toLocaleString('zh-CN')}</td>
                <td style={S.td}>
                  {c.status === 'active' ? (
                    <>
                      <button onClick={() => setVoidTarget(c)} disabled={busy !== null} style={btnStyle('#EF4444')}>退款作废</button>
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: 8 }}>
          <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} style={btnStyle('#374151')}>上一页</button>
          <span style={{ color: '#9CA3AF', fontSize: 13, alignSelf: 'center' }}>第 {page + 1} / {totalPages} 页</span>
          <button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1} style={btnStyle('#374151')}>下一页</button>
        </div>
      )}
...
function StatCard({ label, value, accent }: { label: string; value: string; accent: string }) {
  return (
    <div style={{
      background: '#0F172A', border: '1px solid #1F2937', borderRadius: 12, padding: '16px 18px',
      position: 'relative', overflow: 'hidden',
    }}>
      <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 3, background: accent }} />
      <p style={{ color: '#6B7280', fontSize: 12, margin: 0 }}>{label}</p>
      <p style={{ color: '#E5E7EB', fontSize: 22, fontWeight: 700, margin: '6px 0 0', letterSpacing: 0.5 }}>{value}</p>
    </div>
  )
}
...
function VoidModal({ claim, busy, onClose, onConfirm }: {
  ...
  return (
    <Overlay onClose={onClose}>
      <div style={modalCard}>
        <h3 style={{ color: '#E5E7EB', fontSize: 18, fontWeight: 700, margin: '0 0 4px' }}>退款作废</h3>
        ...
        <label style={{ ...labelStyle, marginTop: 16 }}>退款比例：<b style={{ color: '#C2410C' }}>{ratio}%</b></label>
        ...
        <div style={{ display: 'flex', gap: 16, marginTop: 10, fontSize: 13, color: '#9CA3AF' }}>
          <span>回滚通宝 TB：<b style={{ color: '#F59E0B' }}>{backTb.toFixed(2)}</b></span>
          <span>回滚贡献值 CV：<b style={{ color: '#A5B4FC' }}>{backCv.toFixed(4)}</b></span>
        </div>

        <div style={{ display: 'flex', gap: 10, marginTop: 22 }}>
          <button onClick={onClose} disabled={busy} style={{ flex: 1, padding: '11px', background: 'transparent', border: '1px solid #374151', borderRadius: 8, color: '#9CA3AF', fontSize: 14, cursor: 'pointer' }}>取消</button>
          <button onClick={() => onConfirm(reason, ratio)} disabled={busy} style={{ flex: 2, padding: '11px', background: busy ? '#7A2508' : 'linear-gradient(135deg,#C2410C,#EA580C)', border: 'none', borderRadius: 8, color: '#fff', fontSize: 14, fontWeight: 600, cursor: busy ? 'not-allowed' : 'pointer' }}>
            {busy ? '处理中...' : `确认作废（${ratio}%）`}
          </button>
        </div>
      </div>
...
function BanModal({ claim, busy, onClose, onConfirm }: {
  ...
  return (
    <Overlay onClose={onClose}>
      <div style={modalCard}>
        <h3 style={{ color: '#EF4444', fontSize: 18, fontWeight: 700, margin: '0 0 4px' }}>违规封禁用户</h3>
        <p style={{ color: '#9CA3AF', fontSize: 13, margin: '0 0 16px' }}>
...
const inputStyle: React.CSSProperties = {
  ...
  border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, color: '#E5E7EB', fontSize: 14,
  outline: 'none', boxSizing: 'border-box',
}
const labelStyle: React.CSSProperties = { color: '#9CA3AF', fontSize: 13, fontWeight: 500, display: 'block', marginBottom: 8 }
