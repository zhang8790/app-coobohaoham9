...
这些是 Pretext 创意 demo 的常用模式。每个片段都可以放进 HTML 的 `<script type="module">` 中使用，前提是已从 `https://esm.sh/@chenglou/pretext@0.0.6` 引入对应 API。

## 目录

- [1. Flow around an obstacle (variable-width column)](#1-flow-around-an-obstacle-variable-width-column)
- [2. Text-as-geometry game (word-bricks with collision)](#2-text-as-geometry-game-word-bricks-with-collision)
- [3. Shatter / explode typography](#3-shatter--explode-typography)
- [4. ASCII mask as moving obstacle](#4-ascii-mask-as-moving-obstacle)
- [5. Editorial multi-column with shared cursor](#5-editorial-multi-column-with-shared-cursor)
- [6. Multiline shrink-wrap (tightest-fitting card)](#6-multiline-shrink-wrap-tightest-fitting-card)
- [7. Kinetic typography](#7-kinetic-typography)
- [8. Font stack patterns](#8-font-stack-patterns)
