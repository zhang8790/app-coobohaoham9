...
NODE_OPTIONS= rm -rf dist_new dist node_modules/.vite node_modules/.cache .taro 2>/dev/null || true

echo "[build-weapp] 开始构建 (node 入口, 前台) ..."
NODE_OPTIONS= node node_modules/@tarojs/cli/bin/taro build --type weapp

echo "[build-weapp] 完成。产物在 dist_new/（config/index.ts outputRoot=dist_new，微信开发者工具 project.config.json miniprogramRoot=dist_new/ 已对应）"
