...
const MOCK_DATA = {
  ...
  stores: [
    { id: 'store_001', owner_id: 'usr_002', name: '霸王茶姬（旗舰店）', description: '正宗奶茶，口感醇厚', address: '深圳市南山区科技园', phone: '0755-12345678', category: '餐饮', image_url: '/mock/store1.jpg', banner_url: '/mock/banner1.jpg', rating: 4.8, is_active: true, short_code: 'WC001', created_at: '2026-06-21T10:00:00Z' },
  ],
  products: [
    { id: 'prod_001', store_id: 'store_001', category_id: null, name: '伯牙绝弦', description: '招牌奶茶，香甜可口', price: 18.00, original_price: 22.00, image_url: '/mock/prod1.jpg', stock: 100, barcode: '6931234567890', mood_tags: ['开心', '放松'], scene_tags: ['下午茶', '聚会'], is_active: true, review_status: 'approved', created_at: '2026-06-22T10:00:00Z' },
  ...
  withdrawals: [
    ...
    { id: 'wd_002', user_id: 'usr_001', amount: 100.00, status: 'approved', withdraw_method: 'alipay', bank_name: null, bank_account: null, bank_holder: null, alipay_account: 'zhangwuji@alipay.com', reject_reason: null, created_at: '2026-06-25T14:00:00Z', profiles: { nickname: '张无忌', phone: '13800138001' } },
  ],
  articles: [
    { id: 'art_001', user_id: 'usr_001', title: '霸王茶姬新品体验', content: '今天尝试了伯牙绝弦，口感真的很棒！', is_published: true, created_at: '2026-06-26T10:00:00Z', profiles: { nickname: '张无忌' } },
    { id: 'art_002', user_id: 'usr_002', title: '瑞幸咖啡优惠活动', content: '本周所有饮品第二杯半价！', is_published: false, created_at: '2026-06-28T15:00:00Z', profiles: { nickname: '杨过' } },
  ],
  announcements: [
...
function respond(res: any, status: number, data: any) {
...
const server = createServer(async (req, res) => {
  ...
  const tableData = (MOCK_DATA as any)[tableName]
  if (!tableData) {
  ...
  try {
    if (method === 'GET') {
      ...
      const filterKey = Object.keys(query).find(k => !['select', 'order', 'limit', 'offset', 'count'].includes(k))
      let data = [...tableData]

      // 简单过滤
      if (filterKey) {
        const filterValue = query[filterKey] as string
        data = data.filter((row: any) => {
          const rowValue = row[filterKey]
          if (filterValue.startsWith('eq.')) {
            return rowValue === filterValue.replace('eq.', '')
          }
          return true
        })
      }
      ...
      // 计数
      const count = query.count ? data.length : null

      // 限制
      const limit = parseInt(query.limit as string) || data.length
      const offset = parseInt(query.offset as string) || 0
      data = data.slice(offset, offset + limit)

      const headers: any = { 'Content-Type': 'application/json' }
      if (count !== null) headers['Content-Range'] = `0-${data.length - 1}/${count}`
      ...
      const index = tableData.findIndex((row: any) => row.id === id)
      if (index >= 0) {
...
server.listen(PORT, () => {
  ...
  console.log(`   3. 管理后台将使用本地 Mock API\n`)
})
