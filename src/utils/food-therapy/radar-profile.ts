...
import { HEALTH_TAGS, NATURE_SCALE } from './types'
...
export function buildRadarProfile(products: Product[], term: SeasonalTerm | null): RadarProfile {
  const valid = (products || []).filter(
    (p) => p && p.id && (p.overall_nature || (p.health_tag && p.health_tag.length)),
  )
  ...
  for (const p of valid) {
    const n = p.overall_nature
    if (WARM.has(n)) warm++
    else if (COOL.has(n)) cool++
    for (const t of p.health_tag || []) {
      if ((HEALTH_TAGS as readonly string[]).includes(t)) tagSet.add(t)
    }
    if (n && (NATURE_SCALE as readonly string[]).includes(n)) natureSet.add(n)
    const cat = (p as any).food_category
  ...
  if (goodNatures) {
    for (const p of valid) {
      if (p.overall_nature && goodNatures.has(p.overall_nature)) seasonalHit++
    }
  }
  ...
  const dims: RadarDim[] = [
    ...
    { key: 'pairing', label: DIM_LABELS.pairing, value: clamp01(catSet.size / 4) },
  ]
  ...
  const summary = `你偏爱${natureWord}饮食，已尝过 ${tagSet.size} 类食养功效、${natureSet.size} 种性味；${goodNatures ? `当前「${term!.name}」时令契合度 ${Math.round((seasonalHit / total) * 100)}%` : ''}`

  return { hasData: true, boughtCount: total, dims, summary }
}
