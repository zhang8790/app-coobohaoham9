...
// ---------- 4.4 商品类型闸门：食养/配料是「食品」专属能力 ----------
/**
 * 商品是否可走「食养 / 配料解析」能力。
 *
 * 为什么需要它：食疗、配料安全、适合人群、食性这些都是食品专属结论。
 * 若不按商品类型闸门，工艺品 / 日用品 / 礼品只要带了 therapy_json 或 ingredients，
 * 就会被解析出「适合：xxx」「食性：温」这类荒谬结论（如红枣木手串被算出食养）。
 *
 * 兼容策略：product_kind 为空的历史商品仍按原逻辑放行——该字段是后加的，
 * 存量商品绝大多数是食品，一刀切收紧会导致大量正常食品失去食养展示。
 * 待商家补全类型或做数据回填后，可再把空值收紧为非食品。
 */
export function isFoodProduct(p?: { product_kind?: string | null } | null): boolean {
  if (!p) return false
